...
{
    internal class NewReleaseDataController
    {
        ...
        public NewReleaseDataController(IDeezerSession session)
        {
        ...
        public void SetGenreId(ulong genreId)
        {
            ...
            ItemFetcher<IAlbumViewModel> fetcher = (int startingIndex, int numberOfItems, CancellationToken token)
                => this.session.Genre.GetNewReleasesForGenre(this.genreId, token, (uint)startingIndex, (uint)numberOfItems)
        ...
        public IObservableCollection<IAlbumViewModel> NewReleases => this.albumCollection;
    }
}
